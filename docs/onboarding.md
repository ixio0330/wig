...
- 대시보드 차트/시각화 고도화 미완료
- 프로필 탈퇴 API 미구현
- `dashboard/my`는 날짜 선택 기반 주간/월간 탐색을 지원하며, 월간 본문은 주차별 기록판 UI로 구현되었다

### 2.3. 현재 우선순위 (2026-03-24 결정)

- 단기 우선순위는 `유료화`가 아니라 `범용 서비스화 + 무료 가치 강화`다.
- 신규 팀이 관리자 개입 없이 진입 가능한 셀프서브 흐름을 우선 구축한다.
- 무료 사용자 기준 핵심 여정(설정 → 기록 → 주간 리뷰)이 끊기지 않는 구조를 먼저 완성한다.
- 유료화(결제/청구)는 무료 가치와 리텐션 신호가 확인된 이후 단계로 둔다.

### 2.3.1. 워크스페이스 없음 상태 처리 규칙

- `GET /api/workspaces/me`가 `404`를 반환하면 에러가 아니라 정상 온보딩 상태로 처리한다.
- 프론트는 아래 2개 CTA를 동시에 제공한다.
  - `워크스페이스 만들기` (`POST /api/workspaces`)
  - `참가하기` (`POST /api/workspaces/join-by-invite`, 도입 전에는 `/api/workspaces/join` 임시 사용 가능)
...
- 탈퇴는 아직 서버 삭제 API 없이 UI에서도 비활성 상태다

### 6.8. Notification / Push

- `/api/push/subscribe`, `/api/push/send-daily` 라우트 존재
...
3. 대시보드 차트/시각화 고도화
4. Push/보조 API를 OpenAPI 계약 및 공통 응답 패턴으로 정리할지 결정
5. 탈퇴/계정 삭제 정책을 문서와 구현에서 함께 확정
