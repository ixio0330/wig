...
- `dashboard/my` 주간 점수판에서 `미기록 -> O -> X -> 미기록` 토글 가능
...
- `dashboard` 팀 뷰는 여전히 mock 데이터 의존
- `measure/[id]`는 여전히 mock 데이터 의존
- 대시보드 차트/시각화 고도화 미완료
...
- 사용자 닉네임은 별도 `me` API가 아직 없어 일부 화면에서 `localStorage.wig_user`를 임시 사용한다
...
- `src/app/dashboard/my/page.tsx`
  - 내 대시보드
- `src/app/dashboard/my/_hooks/useDashboardScoreboard.ts`
  - 내 대시보드 API 조회/토글 로직
- `src/app/dashboard/page.tsx`
  - 팀 대시보드 프로토타입
- `src/app/setup/page.tsx`
  - 점수판/선행지표 설정 화면
- `src/app/setup/_hooks/useScoreboardSetup.ts`
  - 점수판 설정 관련 로직
- `src/app/measure/[id]/page.tsx`
  - 선행지표 상세 페이지, 현재는 mock 의존
...
- 활성 점수판 조회 구현
- 생성 / 수정 / 아카이브 API 구현
- 프론트는 setup 화면과 dashboard 일부가 연결되어 있음
...
- `src/app/layout.tsx`
  - QueryClientProvider, ToastProvider, MockDataProvider 등록
- `src/api/mutator.ts`
  - 쿠키 인증과 응답 래핑 동작
...
- 로그인/세션 문제: `src/lib/server/auth.ts`, `src/app/api/auth/*`, `src/context/MockDataContext.tsx`
...
- 대시보드 mock 제거: `src/app/dashboard/page.tsx`, `src/app/measure/[id]/page.tsx`, `src/context/MockDataContext.tsx`
...
2. `measure/[id]` mock 제거
3. 사용자 `me` 조회/수정 API 정리 후 닉네임 임시 처리 제거
4. 전역 TypeScript 오류 정리
5. `yarn lint` 스크립트 정상화
6. 대시보드 시각화와 confetti 인터랙션 추가
