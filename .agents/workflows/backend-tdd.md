...
3. **서버 검증 우선**: 클라이언트 유효성 검사와 관계없이 API 레벨에서 모든 규칙을 독립적으로 검증한다. **검증 스키마는 반드시 [Zod](https://zod.dev/)를 사용하여 정의한다.**
...
코드의 책임과 테스트 용이성을 위해 다음과 같이 레이어를 분리한다.

1.  **Controller (Route Handlers)**: `src/app/api/`
    - 역할: HTTP 요청 수신, **Zod를 통한 입력값 검증**, 서비스 호출, 표준 응답 반환.
    - 테스트: 통합 테스트 (API 엔드포인트).
2.  **Service**: `src/domain/*/services/`
    - 역할: 핵심 비즈니스 로직 수행, 여러 Storage 호출 및 데이터 가공.
    - 테스트: 단위 테스트 (Storage 모킹).
3.  **Storage (Repository)**: `src/domain/*/storage/`
    - 역할: DB 접근 로직 (Drizzle ORM 전용). 특정 테이블의 CRUD 책임.
    - 테스트: 단위 테스트 또는 DB 통합 테스트.

---

### OpenAPI 작성 가이드
1. **파일 위치**: `src/api-spec/openapi.yaml`에 모든 도메인 통합 명세를 관리하거나, 도메인별로 분할 관리한다.
2. **구조적 규칙 (Orval 호환성)**:
   - `components.responses`는 `components.schemas`와 **동등한 레벨(Siblings)**이어야 한다. `schemas` 하위에 중첩되지 않도록 주의한다.
   - 모든 에러 응답 구조는 `ErrorResponse` 스키마를 정의하여 참조(`$ref`)한다.
3. **태그 전략**: 각 엔드포인트에 `tags`를 부여한다. Orval은 태그 단위로 클라이언트 파일을 분할 생성(`mode: tags-split`)한다.
4. **보안 정의**: 공통 세션 쿠키(`wig_sid`)는 `components.securitySchemes`에 정의하고, 보호가 필요한 API의 `security` 필드에서 참조한다.
